...
if ( ! class_exists( 'Percon_Master' ) ) :
	class Percon_Master {
		...
		public function load_wp_scripts_and_styles() {
			get_template_part( 'inc/classes/frontend/partials/action-wp-enqueue-scripts' );
		}
		...
		public function page_title( $title, $sep = ' | ' ) {
			...
			$filtered_title .= ( 2 <= $paged || 2 <= $page ) ? $sep . sprintf( __( 'Page %s', 'percon' ), max( $paged, $page ) ) : '';
